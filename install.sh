...
print_securely_wipe_disk_notice() {
  ...
  echo ""
  echo "then"
  echo "Create encrypted partitions, then fill with /dev/urandom (dd if=/dev/urandom of=/path/to/file for example),"
  ...
  echo "This script will perform this prior to installing hArch."
  echo ""
  echo "then"
  echo "complete your installation with full disk encryption, ensure TRIM is disabled, it should be disabled by default."
  echo "Normally you would want to fill in the space afterwards, but that is when you aren't filling an encrypted disk prior to installing."
  echo ""
